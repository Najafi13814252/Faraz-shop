...
const categories: CategoryShape[] = [
    ...
    { id: 6, img: '/images/acc.png', title: 'لوازم جانبی', categoryName: 'accessories' }
]
    ...
    return (
        <div className="mt-8 grid grid-cols-3 sm:flex sm:items-center sm:justify-between p-2 md:p-0 gap-6 overflow-x-scroll overflow-y-hidden md:overflow-visible">
            {categories.map(category => (
                <Link href={`/product-category/${category.categoryName}`} className="flex flex-col flex-shrink-0 items-center gap-3.5 cursor-pointer hover:scale-110 duration-200" key={category.id}>
