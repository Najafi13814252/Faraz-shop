    ...
    return (
        <>
            <div className="flex flex-col gap-6 md:gap-12 mt-10">
                ...
                <div className="grid grid-cols-3 sm:flex sm:items-center sm:justify-between p-2 md:p-0 gap-12 overflow-x-scroll overflow-y-hidden md:overflow-visible">
                    {categories.map(category => (
                        <Link href={`/product-category/${category.categoryName}`} className="flex flex-col flex-shrink-0 items-center gap-4 cursor-pointer" key={category.id}>
                            <Image className="bg-sky-50 p-2 outline-[3px] outline-offset-[3px] outline-sky-600 rounded-full hover:scale-110 hover:outline-[3.5px] duration-200 w-20 h-20 md:w-32 md:h-32" src={category.img} width={128} height={128} alt={category.title} />
