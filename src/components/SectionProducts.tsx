...
export default function SectionProducts({ children, discountBox, title }: { children: React.ReactNode, discountBox?: boolean, title: string }) {
    return (
        <section className="my-8">
            <div className={`w-full h-full rounded-lg ${discountBox ? "border-4 border-red-500 p-3 bg-red-50/50" : "bg-sky-50/50 border-2 border-sky-700 px-3 py-1"}`}>
                <div className={`flex justify-between items-center ${discountBox ? "bg-red-500 text-white p-2 rounded-lg" : "text-sky-800 border-b border-gray-300 px-2 pt-2 pb-3.5"}`}>
                    {discountBox ? (<div className="flex items-center gap-1">
                        <Icon className="text-2xl md:text-4xl" icon="solar:sale-outline" />
                        <span className="font-medium text-lg md:text-2xl">{title}</span>
                    </div>) : (<span className="font-medium text-lg md:text-xl">{title}</span>)}

                    <button className="flex items-center font-medium cursor-pointer">
                        نمایش همه
                        <Icon className="text-lg md:text-2xl" icon="solar:alt-arrow-left-outline" />
                    </button>
                </div>
